...
- **Master username:** `<set me>`
- **Master password:** `<set me>`
...
```bash
kubectl run mysql-client --rm -it --image=mysql:8.4 --restart=Never -- bash
mysql -h mydb101.cifguuuaskpf.us-east-1.rds.amazonaws.com -u <username> -p
...
Inside the MySQL shell, create the `usermgmt` schema:
